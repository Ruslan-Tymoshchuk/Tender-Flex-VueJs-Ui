    ...
    <v-window-item value="tenderDescription">
      ...
      <v-container class="details-container">
        <v-row class="text-center mx-10">
          ...
          <v-col class="text-left mx-2">
            ...
            <div class="mt-3">{{ contract.minPrice }}</div>
            <div class="mt-3">{{ `${contract.currency.code} | ${contract.currency.symbol}` }}</div>
          </v-col>
          <v-col class="text-left mx-2">
          ...
          <v-col class="text-left mx-2">
            <div>{{ CONTRACT_TYPE[contract.contractType.title] }}</div>
            <div class="mt-3">{{ contract.maxPrice }}</div>
      ...
      <v-container class="details-container">
        <v-row class="text-center mx-10">
          ...
          <v-col class="text-left mx-2">
            ...
            <div class="mt-3">{{ contract.signedDeadline }}</div>
          </v-col>
          <v-col class="text-left mx-2">
      ...
      <v-container class="details-container">
        <v-item-group class="mx-8">
          <v-row>
            <FileVchip
              :fileName="contract.fileMetadata.name"
              :fileKey="contract.fileMetadata.awsS3fileKey"
              @show-file="showFile"
          ></FileVchip>
          </v-row>
...
export default {
  ...
  async mounted() {
    const [tenderResponse, contractResponse] = await Promise.all([
        this.fetchFromEndpoint(`${URL_REST_API.HOST}/${URL_REST_API.TENDERS}/${this.$route.query.tender_id}`),
        this.fetchFromEndpoint(`${URL_REST_API.HOST}/${URL_REST_API.CONTRACTS}/${this.$route.query.contract_id}`)
      ]);
