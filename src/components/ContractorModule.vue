<template>
  <v-toolbar color="blue" extended extension-height="170">
    <v-row>
      ...
      <v-chip href="/tenders-list" class="my-2 ml-12" variant="flat" color="blue-darken-3" text-color="white" prepend-icon="mdi-pulse">
        Tenders {{ tenders }}
      </v-chip>
      <v-chip href="/offers-list" class="my-2 ml-10" variant="text" text-color="white" prepend-icon="mdi-message-processing-outline">
        Offers {{ offers }}
      </v-chip>
      <v-spacer></v-spacer>
    ...
    <template v-slot:extension>
      <v-container class="px-15">
        <v-toolbar-title class="mx-10 mb-10" style="font-size: 2rem">Contractors Module</v-toolbar-title>
      </v-container>
    </template>
  </v-toolbar>
